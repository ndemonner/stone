...
describe Stone::Resource do
  ...
  it "should delete an object and it's yaml file upon Resource.delete" do
    author = Author.new(:name => "Foo Bar", :email => "foo@bar.com", :favorite_number => 3)
  ...
  it "should return a previous and next record by ID" do
    post_one = Post.new( :title => "First Post", :body => "Bar" )
    post_two = Post.new( :title => "After First Post", :body => "Bar" )
    post_one.next.should == post_two
    # post_one.next_by_title.should == post_two
    post_two.prev.should == post_one
